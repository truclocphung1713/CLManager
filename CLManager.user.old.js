    ...
    (function () {
        ...
        function bindGalleryVisitedIndicatorLocal(element, threadUrl, variant) {
        ...
        function bindGalleryVisitedIndicator(element, threadUrl, variant) {
            ...
            if (globalObj.CLM && typeof globalObj.CLM.bindGalleryVisitedIndicator === 'function') {
                return globalObj.CLM.bindGalleryVisitedIndicator(element, threadUrl, variant);
            }
            return bindGalleryVisitedIndicatorLocal(element, threadUrl, variant);
        }
        ...
        function createGalleryOverlayLocal(ctx) {
            // 从上下文中解构依赖（如果有的话），否则使用闭包中的变量
            const _injectStyle = ctx?.injectStyle || injectStyle;
            const _isMobilePage = ctx?.isMobilePage || isMobilePage;
            const _normalizeThreadKey = ctx?.normalizeThreadKey || normalizeThreadKey;
            const _hasDownloadedThread = ctx?.hasDownloadedThread || hasDownloadedThread;
            const _subscribeDownloadStatus = ctx?.subscribeDownloadStatus || subscribeDownloadStatus;
            const _handleThreadDownloadButtonClick = ctx?.handleThreadDownloadButtonClick || handleThreadDownloadButtonClick;
            const _clearGallerySourceHighlight = ctx?.clearGallerySourceHighlight || clearGallerySourceHighlight;
            const _closeInlineDownloadWindowIfOpen = ctx?.closeInlineDownloadWindowIfOpen || closeInlineDownloadWindowIfOpen;
            const _updateQualityBadgeElement = ctx?.updateQualityBadgeElement || updateQualityBadgeElement;
            _injectStyle(`
                .clm-gallery-overlay {
                    position: fixed;
                    inset: 0;
            ...
            // 手机端评论按钮/关闭按钮点击事件
            if (mobileCommentBtn) {
                const mobileCommentsCtx = {
                    overlay,
                    commentsPanel,
                    mobileCommentBtn,
                    mobileCommentCloseBtn
                };

                const globalObj = window;
                if (globalObj.CLM && typeof globalObj.CLM.registerMobileComments === 'function') {
                    try {
                        globalObj.CLM.registerMobileComments(mobileCommentsCtx);
                    } catch (e) {
                        console.warn('草榴Manager: 綁定手機端評論抽屜事件失敗', e);
                    }
                } else {
                    globalObj.CLM_PENDING_MOBILE_COMMENTS_CTX = mobileCommentsCtx;
                }
            }
            ...
            downloadBtn.textContent = _isMobilePage() ? '下载' : '打開下載頁面';
            ...
            function updateGalleryQuality(tag) {
                ...
                _updateQualityBadgeElement(galleryQualityBadge, currentQualityTag);
            }
            ...
            function refreshOverlayDownloadButton() {
                ...
                const downloaded = _hasDownloadedThread(currentThreadKey);
                downloadBtn.classList.toggle('clm-downloaded', downloaded);
                downloadBtn.disabled = false;
                downloadBtn.textContent = downloaded ? '已下載' : (_isMobilePage() ? '下载' : '打開下載頁面');
            }
            ...
            function updateDownloadAction(downloadInfo) {
                ...
                if (currentThreadKey && currentDownloadInfo && currentDownloadInfo.pageUrl) {
                    galleryDownloadUnsubscribe = _subscribeDownloadStatus(currentThreadKey, () => refreshOverlayDownloadButton());
                }
            ...
            function handleDownloadClick() {
                ...
                _handleThreadDownloadButtonClick(downloadBtn);
            }
            ...
            function closeOverlay() {
                ...
                _clearGallerySourceHighlight();
                _closeInlineDownloadWindowIfOpen();
            }
            ...
            function openOverlay(newItems, options = {}) {
                ...
                renderViewerAds(ads);
                currentThreadKey = threadUrl ? _normalizeThreadKey(threadUrl) : null;
            ...
            document.addEventListener('keydown', (ev) => {
                ...
                if (ev.key === 'ArrowRight') {
                    ...
                    ev.preventDefault();
                    if (!_closeInlineDownloadWindowIfOpen()) {
                        closeOverlay();
                    }
                }
            });
        ...
        function createGalleryOverlay(ctx) {
            const globalObj = window;
            if (globalObj.CLM && typeof globalObj.CLM.createGalleryOverlay === 'function') {
                try {
                    return globalObj.CLM.createGalleryOverlay(ctx || {
                        createGalleryOverlayFactory: createGalleryOverlayLocal,
                        injectStyle,
                        isMobilePage,
                        normalizeThreadKey,
                        hasDownloadedThread,
                        subscribeDownloadStatus,
                        handleThreadDownloadButtonClick,
                        clearGallerySourceHighlight,
                        closeInlineDownloadWindowIfOpen,
                        updateQualityBadgeElement
                    });
                } catch (e) {
                    console.warn('草榴Manager: 遠程 gallery.createGalleryOverlay 失敗，回退本地實現', e);
                }
            }
            return createGalleryOverlayLocal(ctx);
        }
        ...
        function createSearchDialogLocal() {
        ...
        function createSearchDialog() {
            const globalObj = window;
            if (globalObj.CLM && typeof globalObj.CLM.createSearchDialog === 'function') {
                try {
                    return globalObj.CLM.createSearchDialog({
                        createSearchDialogFactory: createSearchDialogLocal,
                        loadSearchSettings,
                        saveSearchSettings
                    });
                } catch (e) {
                    console.warn('草榴Manager: 遠程 createSearchDialog 失敗，回退本地實現', e);
                }
            }
            return createSearchDialogLocal();
        }

        function createInlineDownloadWindowLocal() {
            const mask = document.createElement('div');
        ...
        function createInlineDownloadWindow() {
            const globalObj = window;
            if (globalObj.CLM && typeof globalObj.CLM.createInlineDownloadWindow === 'function') {
                try {
                    return globalObj.CLM.createInlineDownloadWindow({
                        createInlineDownloadWindowFactory: createInlineDownloadWindowLocal
                    });
                } catch (e) {
                    console.warn('草榴Manager: 遠程 createInlineDownloadWindow 失敗，回退本地實現', e);
                }
            }
            return createInlineDownloadWindowLocal();
        }
        ...
        // 設置 search 模塊上下文
        (function setupSearchModuleBridge() {
            const globalObj = window;
            const searchModuleCtx = {
                createSearchDialogFactory: createSearchDialogLocal,
                loadSearchSettings,
                saveSearchSettings
            };
            if (globalObj.CLM && typeof globalObj.CLM.initSearchModule === 'function') {
                try {
                    globalObj.CLM.initSearchModule(searchModuleCtx);
                } catch (e) {
                    console.warn('草榴Manager: 初始化 search 模塊失敗', e);
                }
            } else {
                globalObj.CLM_PENDING_SEARCH_CTX = searchModuleCtx;
            }
        })();

        const galleryOverlay = createGalleryOverlay();

        // 將畫廊上下文傳遞給遠程 gallery 模塊（如存在）
        (function setupGalleryModuleBridge() {
            const globalObj = window;
            const galleryModuleCtx = {
                fetchThreadData,
                focusGallerySource,
                clearGallerySourceHighlight,
                markThreadGalleryVisited,
                getCurrentListHoverCtx: () => currentListHoverCtx,
                galleryOverlay,
                createGalleryOverlayFactory: createGalleryOverlayLocal,
                injectStyle,
                isMobilePage,
            ...
            if (globalObj.CLM && typeof globalObj.CLM.initGalleryModule === 'function') {
                try {
                    globalObj.CLM.initGalleryModule(galleryModuleCtx);
                } catch (e) {
                    console.warn('草榴Manager: 初始化 gallery 模塊失敗', e);
                }
            } else {
                globalObj.CLM_PENDING_GALLERY_CTX = galleryModuleCtx;
            }
        })();
        ...
        // 設置 download 模塊上下文
        (function setupDownloadModuleBridge() {
            const globalObj = window;
            const downloadModuleCtx = {
                createInlineDownloadWindowFactory: createInlineDownloadWindowLocal
            };
            if (globalObj.CLM && typeof globalObj.CLM.initDownloadModule === 'function') {
                try {
                    globalObj.CLM.initDownloadModule(downloadModuleCtx);
                } catch (e) {
                    console.warn('草榴Manager: 初始化 download 模塊失敗', e);
                }
            } else {
                globalObj.CLM_PENDING_DOWNLOAD_CTX = downloadModuleCtx;
            }
        })();
        ...
        async function openGalleryForThreadLocal(threadUrl, options = {}) {
            if (!threadUrl) return null;
        ...
        async function openGalleryForThread(threadUrl, options = {}) {
            const globalObj = window;
            if (globalObj.CLM && typeof globalObj.CLM.openGalleryForThread === 'function') {
                try {
                    return await globalObj.CLM.openGalleryForThread(threadUrl, options);
                } catch (e) {
                    console.warn('草榴Manager: 遠程 gallery 打開失敗，回退本地實現', e);
                }
            }
            return await openGalleryForThreadLocal(threadUrl, options);
        }

        function setupThreadDownloadButtonLocal(btn, options = {}) {
        ...
        function setupThreadDownloadButton(btn, options = {}) {
            const globalObj = window;
            if (globalObj.CLM && typeof globalObj.CLM.setupThreadDownloadButton === 'function') {
                return globalObj.CLM.setupThreadDownloadButton(btn, options);
            }
            return setupThreadDownloadButtonLocal(btn, options);
        }
        ...
        // 画质标签相关功能已迁移到 modules/gallery.js
        // 这里仅保留委托函数，实际实现在模块中

        function detectQualityTagFromTitle(titleText) {
            const globalObj = window;
            if (globalObj.CLM && typeof globalObj.CLM.detectQualityTagFromTitle === 'function') {
                return globalObj.CLM.detectQualityTagFromTitle(titleText);
            }
            console.warn('草榴Manager: gallery 模块未加载，画质标签功能不可用');
            return null;
        }

        function resolveQualityTagFromDocument(doc) {
            const globalObj = window;
            if (globalObj.CLM && typeof globalObj.CLM.resolveQualityTagFromDocument === 'function') {
                return globalObj.CLM.resolveQualityTagFromDocument(doc);
            }
            console.warn('草榴Manager: gallery 模块未加载，画质标签功能不可用');
            return null;
        }
        ...
        function resolveQualityTagFromListItem(wfItem, threadAnchor = null) {
            const globalObj = window;
            if (globalObj.CLM && typeof globalObj.CLM.resolveQualityTagFromListItem === 'function') {
                return globalObj.CLM.resolveQualityTagFromListItem(wfItem, threadAnchor);
            }
            console.warn('草榴Manager: gallery 模块未加载，画质标签功能不可用');
            return null;
        }
        ...
        function updateQualityBadgeElement(badgeEl, tag) {
            const globalObj = window;
            if (globalObj.CLM && typeof globalObj.CLM.updateQualityBadgeElement === 'function') {
                return globalObj.CLM.updateQualityBadgeElement(badgeEl, tag);
            }
            console.warn('草榴Manager: gallery 模块未加载，画质标签功能不可用');
        }
        ...
        if (isMobile && href.indexOf('thread0806.php') !== -1) {
            ...
            const globalObj = window;
            if (globalObj.CLM && typeof globalObj.CLM.initMobileForumEnhancements === 'function') {
                try {
                    globalObj.CLM.initMobileForumEnhancements(mobileForumCtx);
                } catch (e) {
                    console.warn('草榴Manager: 初始化手機端板塊頁失敗', e);
                }
            } else {
                globalObj.CLM_PENDING_MOBILE_FORUM_CTX = mobileForumCtx;
            }
        }
         ...
         * 向頁面注入右下角的設置按鈕與面板
         */
        function createSettingsUILocal() {
            injectStyle(`
                .clm-settings-btn {
                    position: fixed;
                    right: 16px;
        ...
        function createSettingsUI() {
            const globalObj = window;
            if (globalObj.CLM && typeof globalObj.CLM.createSettingsUI === 'function') {
                try {
                    return globalObj.CLM.createSettingsUI({
                        createSettingsUIFactory: createSettingsUILocal,
                        loadSettings,
                        saveSettings,
                        showToast,
                        sendToQb,
                        normalizeSavePresets,
                        openPresetPickerDialog
                    });
                } catch (e) {
                    console.warn('草榴Manager: 遠程 createSettingsUI 失敗，回退本地實現', e);
                }
            }
            return createSettingsUILocal();
        }

        // 設置 settings 模塊上下文
        (function setupSettingsModuleBridge() {
            const globalObj = window;
            const settingsModuleCtx = {
                createSettingsUIFactory: createSettingsUILocal,
                loadSettings,
                saveSettings,
                showToast,
            ...
            if (globalObj.CLM && typeof globalObj.CLM.initSettingsModule === 'function') {
                try {
                    globalObj.CLM.initSettingsModule(settingsModuleCtx);
                } catch (e) {
                    console.warn('草榴Manager: 初始化 settings 模塊失敗', e);
                }
            } else {
                globalObj.CLM_PENDING_SETTINGS_CTX = settingsModuleCtx;
            }
        })();
