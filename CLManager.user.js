    ...
    (function () {
        ...
        function createGalleryOverlay() {
            ...
            if (isMobilePage()) {
                ...
                topicPanel.addEventListener('click', (e) => {
                    ...
                    if (viewportHeight > 0) {
                        const isExpanded = topicPanel.classList.contains('clm-topic-expanded');
                        // 如果抽屉已展开，点击区域扩大到整个抽屉（屏幕下方 75%）
                        // 如果抽屉收起，点击区域只在屏幕下方 25%
                        const bottomThreshold = isExpanded ? viewportHeight * 0.25 : viewportHeight * 0.75;
        ...
        function initSearchPageFeatures() {
            ...
            function convertSearchToCards() {
                // 查找所有 div.t 容器
                const tableContainers = document.querySelectorAll('div.t');
                if (!tableContainers || tableContainers.length === 0) return;
                
                // 遍历所有容器，找到包含搜索结果的那个
                // 搜索结果的特征：tr.tr3 行中包含 htm_data 或 htm_mob 链接
                let targetTable = null;
                let targetContainer = null;
                
                for (const container of tableContainers) {
                    const table = container.querySelector('table');
                    if (!table || table.dataset.clmConverted === '1') continue;
                    
                    // 检查是否包含搜索结果链接
                    const resultLinks = table.querySelectorAll('tr.tr3 a[href*="htm_data"], tr.tr3 a[href*="htm_mob"]');
                ...
                targetTable.dataset.clmConverted = '1';
                targetTable.classList.add('clm-search-converted'); // 添加类以隐藏表格
                
                // 创建卡片网格容器
                const grid = document.createElement('div');
                grid.className = 'clm-search-grid';
                
                // 获取所有搜索结果行 - 只处理包含帖子链接的行
                const rows = targetTable.querySelectorAll('tr.tr3');
                ...
                // 替换表格
                targetTable.parentNode.insertBefore(grid, targetTable);
            }
        ...
        function initMobileGalleryStyles() {
            ...
            injectStyle(`
                ...
                body.clm-mobile-gallery .clm-gallery-download-preview-header {
                    background: #fff !important;
                ...
                body.clm-mobile-gallery .clm-gallery-download-preview-frame {
                    flex: 1 !important;
                    width: 100% !important;
                    height: 100% !important;
                    border: none !important;
                ...
                body.clm-mobile-gallery .clm-gallery-download-preview-footer {
                    background: #fff !important;
        ...
        function initPageSpecificFeatures() {
            console.log('草榴Manager: 页面特定功能初始化（由模块管理）');
            
            // 设置远程模块已加载标志
            remoteModulesLoaded = true;
            
            // 注意：页面特定功能已在脚本初始化时暴露到 CLM 命名空间
            // desktop.js 和 mobile.js 模块会在初始化时调用这些函数
            // 这里不需要再做任何事情
        }
